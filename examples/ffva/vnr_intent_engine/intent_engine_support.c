...
static StreamBufferHandle_t samples_to_engine_stream_buf[2] = {0,0};

void intent_engine_stream_buf_reset(void)
{
    if (samples_to_engine_stream_buf[0])
        while (xStreamBufferReset(samples_to_engine_stream_buf[0]) == pdFAIL)
            vTaskDelay(pdMS_TO_TICKS(1));

    if (samples_to_engine_stream_buf[1])
        while (xStreamBufferReset(samples_to_engine_stream_buf[1]) == pdFAIL)
            vTaskDelay(pdMS_TO_TICKS(1));
}
        ...
        int32_t vnr_pred)
{
    ...
    if(samples_to_engine_stream_buf[0] != NULL) {
        size_t bytes_to_send = sizeof(int32_t) * frame_count;
        if (xStreamBufferSend(samples_to_engine_stream_buf[0], processed_audio_frame, bytes_to_send, 0) != bytes_to_send) {
            rtos_printf("lost local output samples for intent\n");
        }
        if (xStreamBufferSend(samples_to_engine_stream_buf[1], &vnr_pred, sizeof(int32_t), 0) != sizeof(int32_t)) {
            rtos_printf("lost local vnr pred output samples for intent\n");
        }
...
{
    samples_to_engine_stream_buf[0] = xStreamBufferCreate(
                                           appconfINTENT_FRAME_BUFFER_MULT * appconfAUDIO_PIPELINE_FRAME_ADVANCE,
                                           appconfINTENT_SAMPLE_BLOCK_LENGTH);

    samples_to_engine_stream_buf[1] = xStreamBufferCreate(
                                    appconfINTENT_FRAME_BUFFER_MULT * 1,
                                    1);
