...
static void receive_audio_frames(StreamBufferHandle_t input_queue, StreamBufferHandle_t input_vnr_queue,
                                 int32_t *buf, int16_t *buf_short, size_t *buf_short_index, int32_t *vnr_pred);
                                 ...
                                 int32_t *buf, int16_t *buf_short, size_t *buf_short_index, int32_t *vnr_pred)
{
    ...
    do {
        ...
        xStreamBufferReceive(input_vnr_queue,
                             vnr_pred,
                             sizeof(size_t),
                             portMAX_DELAY);
    } while (buf_len > 0);
...
void intent_engine_task(void *args)
{
    ...
    while (1)
    {
        ...
        receive_audio_frames(input_queue, input_vnr_queue, buf, buf_short, &buf_short_index, &vnr_pred);
        ...
        if (asr_error != ASR_OK) {
            if (intent_state != STATE_EXPECTING_WAKEWORD && vnr_pred != 0) {
                xTimerReset(int_eng_tmr, 0);
                rtos_printf("Someone talking\n");
            }
            continue;
        }
        ...
        if (!IS_KEYWORD(word_id) && !IS_COMMAND(word_id)) {
            if (intent_state != STATE_EXPECTING_WAKEWORD && vnr_pred != 0) {
                xTimerReset(int_eng_tmr, 0);
                rtos_printf("Someone talking\n");
            }
